 ...
 * Harness generation.
 *
 * Wraps the player's function in a complete program that reads a case count,
 * then loops: read one case's arguments, call the function, print the answer.
 *
 * The loop is the point. One submission runs every test case, so a 21-case
 * submit is a single request instead of 21 — which is the difference between a
 * usable compiled language and an unusable one on a rate-limited service.
 *
 * Every harness is written against the standard library only — judge sandboxes
 * have no packages available.
 */
import { byId, entryName } from './languages.mjs'
...
const HARNESS = {
  c: (sig, name, code) => {
    ...
    for (const p of sig.params) {
      decls.push(cRead(p.type, p.name))
      call.push(p.name)
      if (p.type === 'int[]' || p.type === 'string[]') call.push(`${p.name}_len`)
      if (p.type === 'int[][]' || p.type === 'string[][]') call.push(`${p.name}_rows`, `${p.name}_cols`)
    }
    const r = sig.returns
    let resultDecl
    if (r === 'int[]') { call.push('&out_len'); resultDecl = 'int out_len = 0;\n    long long* result = ' }
    else if (r === 'int[][]') { call.push('&out_rows', '&out_cols'); resultDecl = 'int out_rows = 0; int* out_cols = NULL;\n    long long** result = ' }
    else if (r === 'string') resultDecl = 'char* result = '
    else if (r === 'bool') resultDecl = 'int result = '
    else resultDecl = 'long long result = '

    return `#include <stdio.h>
#include <stdlib.h>
#include <string.h>

static long long rd_num(void) { long long v = 0; if (scanf("%lld", &v) != 1) v = 0; return v; }
static char* rd_line(void) {
    int len = (int) rd_num();
    int ch;
    while ((ch = getchar()) != '\\n' && ch != EOF) { }
    char* buf = (char*) malloc((size_t) len + 2);
    if (len == 0) { int c2 = getchar(); (void) c2; buf[0] = '\\0'; return buf; }
    if (!fgets(buf, len + 2, stdin)) buf[0] = '\\0';
    size_t n = strlen(buf);
    while (n > 0 && (buf[n-1] == '\\n' || buf[n-1] == '\\r')) buf[--n] = '\\0';
    return buf;
}
...
int main(void) {
    int _cases = (int) rd_num();
    for (int _c = 0; _c < _cases; _c++) {
${decls.map((d) => '        ' + d.split('\n').join('\n    ')).join('\n')}
        ${resultDecl}${name}(${call.join(', ')});
${indent(cWrite(r), 8)}
    }
    return 0;
}
...
function cRead(t, v) {
  switch (t) {
    case 'int': return `long long ${v} = rd_num();`
    case 'bool': return `int ${v} = rd_num() != 0;`
    case 'string': return `char* ${v} = rd_line();`
    case 'int[]': return `int ${v}_len = (int) rd_num();\n    long long* ${v} = (long long*) malloc(sizeof(long long) * (${v}_len ? ${v}_len : 1));\n    for (int i = 0; i < ${v}_len; i++) ${v}[i] = rd_num();`
    case 'string[]': return `int ${v}_len = (int) rd_num();\n    char** ${v} = (char**) malloc(sizeof(char*) * (${v}_len ? ${v}_len : 1));\n    for (int i = 0; i < ${v}_len; i++) ${v}[i] = rd_line();`
    case 'int[][]': return `int ${v}_rows = (int) rd_num();\n    long long** ${v} = (long long**) malloc(sizeof(long long*) * (${v}_rows ? ${v}_rows : 1));\n    int* ${v}_cols = (int*) malloc(sizeof(int) * (${v}_rows ? ${v}_rows : 1));\n    for (int i = 0; i < ${v}_rows; i++) { ${v}_cols[i] = (int) rd_num(); ${v}[i] = (long long*) malloc(sizeof(long long) * (${v}_cols[i] ? ${v}_cols[i] : 1)); for (int j = 0; j < ${v}_cols[i]; j++) ${v}[i][j] = rd_num(); }`
    case 'string[][]': return `int ${v}_rows = (int) rd_num();\n    char*** ${v} = (char***) malloc(sizeof(char**) * (${v}_rows ? ${v}_rows : 1));\n    int* ${v}_cols = (int*) malloc(sizeof(int) * (${v}_rows ? ${v}_rows : 1));\n    for (int i = 0; i < ${v}_rows; i++) { ${v}_cols[i] = (int) rd_num(); ${v}[i] = (char**) malloc(sizeof(char*) * (${v}_cols[i] ? ${v}_cols[i] : 1)); for (int j = 0; j < ${v}_cols[i]; j++) ${v}[i][j] = rd_line(); }`
    default: throw new Error('c read ' + t)
  }
}
function cWrite(t) {
  switch (t) {
    case 'int': return `printf("%lld\\n", result);`
    case 'bool': return `printf("%d\\n", result ? 1 : 0);`
    case 'string': return `printf("%d\\n%s\\n", (int) strlen(result), result);`
    case 'int[]': return `printf("%d\\n", out_len);\nfor (int i = 0; i < out_len; i++) printf("%lld%s", result[i], i + 1 == out_len ? "" : " ");\nprintf("\\n");`
    case 'int[][]': return `printf("%d\\n", out_rows);\nfor (int i = 0; i < out_rows; i++) { printf("%d\\n", out_cols[i]); for (int j = 0; j < out_cols[i]; j++) printf("%lld%s", result[i][j], j + 1 == out_cols[i] ? "" : " "); printf("\\n"); }`
    default: throw new Error('c write ' + t)
  }
}
